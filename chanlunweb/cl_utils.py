import time
from chanlunweb import fun
from chanlunweb.cl_interface import *
from chanlunweb.exchange import exchange
from chanlunweb.file_db import FileCacheDB
from chanlunweb.db import db
...
def cl_data_to_tv_chart(cd: ICL, config: dict, to_frequency: str = None):
    ...
    if config[f"chart_show_bi_mmd"] == "1":
        for m in cd.get_bi_mmds() :
            k = m.klu
            mmd_text = '笔:'
            for m_type in m.type:
                mmd_text += ',' + m_type.value + ('B' if m.is_buy else 'S')

            mmd_infos[fun.datetime_to_int(k.time.toDateTime())]={
                        "points": {"time": fun.datetime_to_int(k.time.toDateTime()), "price": k.low if m.is_buy else k.high},
                        "text": mmd_text,
                    }
                
    if config[f"chart_show_xd_mmd"] == "1":
        for m in cd.get_xd_mmds() :
            k = m.klu
            mmd_text = '段:'
            for m_type in m.type:
                mmd_text += ',' + m_type.value + ('B' if m.is_buy else 'S')
            if fun.datetime_to_int(k.time.toDateTime()) in mmd_infos.keys():
                mt = mmd_infos[fun.datetime_to_int(k.time.toDateTime())]['text']
                mmd_infos[fun.datetime_to_int(k.time.toDateTime())]={
                        "points": {"time": fun.datetime_to_int(k.time.toDateTime()), "price": k.low if m.is_buy else k.high},
                        "text": mt +' | '+mmd_text,
                    }
